...
namespace MSTART_Hiring_Task.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public string UploadFileGoogle (UploadImageViewModel model)
        {
            string uploadFileName = string.Empty;
            ...
            try
            {
                if (model.PictureProfile != null)
                {
                    ...
                    {
                        Name = Guid.NewGuid().ToString() + "_" + model.PictureProfile.FileName
                    };

                    // Create a stream from the uploaded file
                    using (var stream = new MemoryStream())
                    {
                        model.PictureProfile.CopyTo(stream);
                        stream.Seek(0, SeekOrigin.Begin);

                        // Upload the file to Google Drive
                        var request = driveService.Files.Create(fileMetadata, stream, "your/file/mime-type");
                        request.Upload();

                        // Get the uploaded file
                        var file = request.ResponseBody;

                        // Get the file name generated by Google Drive
            ...
            {
                throw new Exception("An unexpected error occurred while uploading the image to Google Drive.", ex);
            }
        ...
        {
            // Use the OAuth client ID and secret obtained from the Google Cloud Console
